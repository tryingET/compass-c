#!/usr/bin/env python3
"""Deterministically validate the portable COMPASS skill package."""
...
def validate_evals() -> None:
    ...
    for case in cases:
        ...
        if not isinstance(case_id, str) or case_id in case_ids:
            fail("evaluation case IDs must be unique strings")
        case_ids.add(case_id)
        rubric = case.get("expected_output", {})
        criteria = rubric.get("criteria")
        if rubric.get("rubric_version") != 1 or not isinstance(criteria, list):
        ...
        if len(criterion_ids) != len(set(criterion_ids)) or not all(
...
def main() -> int:
    try:
        ...
        return 1
    print(json.dumps({"ok": True, "skill": "compass", "status": "structurally_valid"}))
    return 0
